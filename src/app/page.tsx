    ...
    <>
      ...
      <section className="section-padding pt-8 bg-white">
        <div className="max-content">
          {/* Filter tabs */}
          <ScrollReveal>
            <div className="flex items-center gap-1 p-1 bg-nexus-surface rounded-lg border border-black/[0.04] w-fit mb-10">
              {categories.map((cat) => (
                <button
                  key={cat.value}
                  onClick={() => setFilter(cat.value)}
                  className={`relative px-5 py-2 text-[13px] font-medium rounded-md transition-all duration-200
                    ${filter === cat.value ? 'text-nexus-navy' : 'text-nexus-text-secondary hover:text-nexus-navy'}`}
                >
                  {filter === cat.value && (
                    <motion.div
                  ...
                  <Link href={`/portfolio/${project.slug}`} className="group block">
                    <div className="rounded-xl border border-black/[0.04] overflow-hidden
                      ...
                      <div className="relative aspect-[16/10] bg-gradient-to-br from-nexus-surface to-nexus-surface-alt overflow-hidden">
                        ...
                        {/* Hover overlay */}
                        <div className="absolute inset-0 bg-nexus-navy/60 flex items-center justify-center
                      ...
                      <div className="p-5">
                        ...
                        <div className="mt-4 flex flex-wrap gap-1.5">
                          {project.technologies.slice(0, 4).map((tech) => (
                            <span
                              key={tech}
                              className="px-2 py-0.5 text-[11px] font-mono text-nexus-text-tertiary bg-nexus-surface rounded"
                            >
                              {tech}
                            </span>
                          ))}
